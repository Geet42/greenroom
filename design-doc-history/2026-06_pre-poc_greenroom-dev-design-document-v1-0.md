...
> **Archive note:** this is the original pitch/roadmap document, supplied externally and not generated from this repo's git history — see the `design-doc-history/README.md` exception note. It predates the first in-repo `DESIGN.md` (2026-06-17 POC). The ASCII architecture below is this doc's own; the image immediately after it is the earliest *visual* proposed-architecture diagram found for the project, from the companion pitch/sprint deck (`MockBot_Final.pptx`, "Architecture: Interview Session Flow" — the first of two architecture slides in that deck, chosen because it comes first). The stack it depicts (ASP.NET Core orchestrator, Go code judge, Azure AI Foundry, Cosmos DB) was never built as drawn — the actual POC used FastAPI, Supabase, and Groq from day one, as this same document's own Scope section (§3) already shows.

![Proposed architecture: Interview Session Flow](images/2026-06_proposed-architecture-interview-session-flow.png)
